...
def train(epoch):
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if (batch_idx + 1) % 100 == 0:
            ...
            print('| epoch {:3d} | {:5d}/{:5d} batches | ms/batch {:5.2f} | loss {:5.3f} |'.format(
                epoch, batch_idx + 1, len(tr_dataloader),
                elapsed * 1000 / (batch_idx + 1), loss.item()))

    train_loss /= (batch_idx + 1)
    logging_writer.info('-' * 99)
    logging_writer.info('| epoch {:3d} | time: {:5.2f}s | training loss {:5.3f} |'.format(
...
def validate(epoch):
    ...
    logging_writer.info('| epoch {:3d} | time: {:5.2f}s | cv loss {:5.3f} | cv accuracy: {:5.4f} |'.format(
